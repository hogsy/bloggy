...
struct Config
{
	char title[ 64 ];   // name of the blog
	char subtitle[ 64 ];// subtitle to appear below the name
...
static void post_destroy( Post *self )
{
	PL_DELETE( self->body );
	PL_DELETE( self );
}
...
static Post *parse_post_buf( const char *buf )
{
	...
	if ( post->body == NULL )
	{
		printf( "No body for post, discarding!\n" );
		PL_DELETEN( post );
	}
...
static void print_html_footer( FILE *file )
{
	fprintf( file, "<div class=\"footer\">" );
	{
		...
		fprintf( file, "Generated by <a href=\"https://github.com/hogsy/bloggy\">bloggy</a> (%s).</p>", time );
	}
	fprintf( file, "</div>" );
}
...
static void write_html_homepage( void )
{
	...
	if ( file == NULL )
	{
		printf( "Failed to write homepage!\n" );
		return;
	}

	print_html_header( file, config.title, config.url );
...
static void write_html_pages( void )
{
	...
	for ( unsigned int i = 0; i < numPages; ++i )
	{
		...
		char title[ sizeof( config.title ) * 3 ];
	...
	for ( unsigned int i = 0; i < numPosts; ++i )
	{
		...
		char title[ sizeof( config.title ) * 4 ];
		snprintf( title, sizeof( title ), "%s | %s", config.title, ( *posts[ i ]->title != '\0' ) ? posts[ i ]->title : posts[ i ]->id );

		char url[ sizeof( config.url ) * 2 ];
		snprintf( url, sizeof( url ), "%s/%s.htm", config.url, posts[ i ]->id );
